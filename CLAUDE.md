...
Three self-contained OS zones — `linux/`, `mac/`, `windows/` — in one
repository, each provisioning a machine from scratch with one script.
...
change to one does not propagate — `linux/config/nvim` and `mac/config/nvim`
...
Linux config, and `mac/setup.sh` could not run unless `linux/` existed. When a
...
script before brew exists. `linux/setup.sh` has no such constraint.
