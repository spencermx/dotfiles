...
chat.setup({
    ...
    sticky = {'#buffers'},
    mappings = {reset = {normal = '', insert = ''}, show_diff = {full_diff = true}},
    prompts = {
    ...
    providers = {
        ...
        lmstudio = {
            ...
            get_url = function() return 'http://localhost:1234/v1/chat/completions' end
        }
    }
})
...
vim.keymap.set({'n', 'v'}, '<leader>aq', function()
    vim.ui.input({prompt = 'AI Question> '},
                 function(input) if input ~= '' then chat.ask(input) end end)
